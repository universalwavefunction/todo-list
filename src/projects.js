...
const projects = () => {
  ...
  const addProject = () => {
    const newProject = project(
      `<input type="text" id="edit-project-${projectCount}" placeholder="Project" required>`,
      `<input type="date" id="edit-projectdate-${projectCount}" name="duedate" min="${today}">`,
      false,
      projectCount,
      false
    );
  ...
  const updateProjectDisplay = () => {
    ...
    for (let i = 0; i < myProjects.length; i++) {
      myProjects[i].display =
        "<div class='" +
        myProjects[i].projectClass +
        "' id='" +
        myProjects[i].id +
        "'>" +
        "<div>" +
    ...
    projectEditButtons.forEach((editButton, i) => {
      editButton.addEventListener("click", () => {
        if (myProjects[i].editing) {
          myProjects[
            i
          ].title = `<input type='text' class='editing-project' id='edit-project-${myProjects[i].position}'>`;
          updateProjectDisplay();
          myProjects[i].editing = false;
        } else if (!myProjects[i].editing) {
          const editProjectTitle = document.getElementById(
            `edit-project-${myProjects[i].position}`
          );
          myProjects[i].title =
            "<button id='project-button' class='side-button'>" +
