  ...
  const addTask = () => {
    if (myTasks.length == 0 || myTasks[myTasks.length - 1].editing == true) {
      ...
      const editTitle = document.getElementById(
        `edit-title-${myTasks[editIndex].position}`
      );
      const editDate = document.getElementById(
        `edit-date-${myTasks[editIndex].position}`
      );
  ...
  const updateTaskDisplay = () => {
    for (let i = 0; i < myTasks.length; i++) {
      myTasks[i].display =
        ...
        myTasks[i].id +
        "'>" +
        "<div id='newtask-completed'>" +
        "<input type='checkbox' id='toggle'" +
        myTasks[i].checked +
        ">" +
        myTasks[i].title +
        "</div>" +
        "<div id='newtask-duedate'>" +
        myTasks[i].duedate +
        "</div>" +
        "<div class='buttons'>" +
        "<button id='edit'>" +
        `<img src="edit-button.svg" alt="Edit" height="12px">` +
        "</button>" +
        "<button id='delete'>" +
        "x" +
        "</button>" +
    ...
    edit.forEach((editButton, i) => {
      editButton.addEventListener("click", () => {
        ...
        if (myTasks[editIndex].editing) {
          myTasks[
            editIndex
          ].title = `<input type='text' id='edit-title-${myTasks[editIndex].position}' size='15' value="${myTasks[editIndex].title}">`;
          myTasks[
            editIndex
          ].duedate = `<input type='date' id='edit-date-${myTasks[editIndex].position}' min=${today} value="${myTasks[editIndex].duedate}">`;
          updateTaskDisplay();
